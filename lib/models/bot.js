...
module.exports = function() {
    ...
    if(params && params.length>0) {
      ...
      .then(function(data) {
        if(data && data[0] && data[0].meta) {
          ...
          e.message.channel.sendMessage('Adding "' + feed.title + '" to the watch list.');
        }
        else {
          e.message.channel.sendMessage('Error while adding this feed.');
        }
      })
      .catch(function(e) {
        e.message.channel.sendMessage('Error while adding this feed.');
  ...
  this.doList = function(e, params) {
    ...
    for(var i=0; i<nb; i++) {
      var f = that.feeds[i];
      e.message.channel.sendMessage(i + ' - ' + f.title + ' : ' + f.description);
    }
  };
  ...
  this.doRemove = function(e, params) {
    if(params && params.length>0) {
      ...
      e.message.channel.sendMessage('Removed "' + removed.title + '" from my watch list.');
    }
  };
