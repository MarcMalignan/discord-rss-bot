...
module.exports = function() {
  ...
  this.onReady = function() {
    client.Dispatcher.removeAllListeners(Discordie.Events.GATEWAY_READY);
  ...
  this.onDisconnected = function() {
    client.Dispatcher.removeAllListeners(Discordie.Events.DISCONNECTED);
    client.Dispatcher.on(Discordie.Events.DISCONNECTED, function(e) {
      ...
      // stop workers
      client.Guilds.forEach(function(g) {
        if(that.feeds && that.feeds[g.id] && that.feeds[g.id].length) {
          for(var i=0; i<that.feeds[g.id].length; i++) {
            that.feeds[g.id][i].stopWorker();
          }
        }
      });
  ...
  // -------------------- MESSAGE EVENT -------------------- //
  this.onMsg = function() {
    client.Dispatcher.removeAllListeners(Discordie.Events.MESSAGE_CREATE);
