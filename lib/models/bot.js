...
module.exports = function() {
  ...
  this.init = function() {
    ...
    this.tasks = {
      'add':            this.doAdd,
      'add-tags':       this.doAddTags,
      'list':           this.doList,
      'remove':         this.doRemove,
      'remove-tags':    this.doRemoveTags,
      'sing':           this.doSing,
    };
  ...
  this.doList = function(e, params) {
    ...
    switch(nb) {
      case 0:   that.msg(e, 'I\'m not watching any feeds.');
                break;
      case 1:   that.msg(e, 'I\'m watching 1 feed.');
                break;
      default:  that.msg(e, 'I\'m watching ' + nb + ' feeds.');
    }

    for(var i=0; i<nb; i++) {
      var f = that.feeds[i];
      var msg = i + ' - ' + f.title + ' - ' + f.url;
      if(f.tags.length > 0) {
        msg += ' [' + f.tags.toString() + ']';
      }
      that.msg(e, msg);
