...
module.exports = function(client) {
  ...
  this.doList = function(channel, params) {
    ...
    var msg = '';
    var feeds = that.feeds[channel.guild_id];
    ...
    if(params && params.length>0) {
      ...
      .then(function(data) {
        if(data && data[0] && data[0].meta) {
          ...
          // init server feeds
          if(!that.feeds[channel.guild_id]) {
            that.feeds[channel.guild_id] = [];
          }

          // add feed
          that.feeds[channel.guild_id].push(feed);
  ...
  this.doListRoles = function(channel, params) {
    var serverRoles = that.roles[channel.guild_id];
  ...
  this.doAddRoles = function(channel, params) {
    ...
    // init server roles
    if(!that.roles[channel.guild_id]) {
      that.roles[channel.guild_id] = [];
    }

    // add roles
    for(var i=0; i<roles.length; i++) {
      var role = roles[i].toLowerCase();
      that.roles[channel.guild_id].push(role);
    }
